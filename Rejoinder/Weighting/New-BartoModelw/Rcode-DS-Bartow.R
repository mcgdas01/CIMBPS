...
for(i.ssize in 1:length(nvec)){
  ...
  for(irep in 1:nrep){
	...
	Y <- rpois(n,Z + X1)
    	...
    	#MCMC on Bartolucci model with CF weights
	...
	gam.est<-coef(fit0)
	var.gam<-summary(fit0)$cov.scaled
	old.gam0<-gam.est[1]
	old.gam1<-gam.est[2]
	old.pi<-expit(old.gam0+old.gam1*X)
	old.w<-Z/old.pi+(1-Z)/(1-old.pi)
	old.glike<-sum((1-Z)*log(1-old.pi)+Z*log(old.pi))
	old.gprior<--0.5*prec0*(old.gam0^2+old.gam1^2)
	...
	v0<-log(runif(nits))
	v1<-log(runif(nits))
	y0<-rnorm(nits)*sqrt(var.gam[1,1])
	y1<-rnorm(nits)*sqrt(var.gam[2,2])
	...
	for(iter in 1:nits){
		...
		#Update gammas in cutfeedback style

		new.gam0<-old.gam0+y0[iter]
		new.gam1<-old.gam1
		new.pi<-expit(new.gam0+new.gam1*X)

		new.glike<-sum((1-Z)*log(1-new.pi)+Z*log(new.pi))
		new.gprior<--0.5*prec0*(new.gam0^2+new.gam1^2)
		if(v0[iter]<new.glike+new.gprior-old.glike-old.gprior){
			old.gam0<-new.gam0
			old.gprior<-new.gprior
			old.glike<-new.glike
		}

		new.gam0<-old.gam0
		new.gam1<-old.gam1+y1[iter]
		new.pi<-expit(new.gam0+new.gam1*X)

		new.glike<-sum((1-Z)*log(1-new.pi)+Z*log(new.pi))
		new.gprior<--0.5*prec0*(new.gam0^2+new.gam1^2)
		if(v1[iter]<new.glike+new.gprior-old.glike-old.gprior){
			old.gam1<-new.gam1
			old.gprior<-new.gprior
			old.glike<-new.glike
		}
