...
for(i.ssize in 1:length(nvec)){
  ...
  for(irep in 1:nrep){
	...
	Y <- rpois(n,Z + X1)
    	...
    	#MCMC on Bartolucci model with equal weights
	...
	n1<-sum(ind)
	n0<-n-n1
	old.like0<-log(old.lam0)*S0-n0*old.lam0
	old.prior0<--0.5*prec0*(log(old.lam0))^2-log(old.lam0)
	old.like1<-log(old.lam1)*S1-n1*old.lam1
	...
	for(iter in 1:nits){
		...
		new.like0<-log(new.lam0)*S0-n0*new.lam0
		...
		new.like1<-log(new.lam1)*S1-n1*new.lam1
